...
def test_transaction_amount_exceeds_balance():
    sender_wallet = Wallet()
    recipient = 'recipient'
    amount = STARTING_BALANCE + 10

    with pytest.raises(Exception, match="Amount exceeds the balance."):
        Transaction(sender_wallet, recipient, amount)
...
def test_transaction_update_exceeds_balance():
    ...
    with pytest.raises(Exception, match="Amount exceeds the balance."):
        transaction.update(sender_wallet, 'new_recipient', STARTING_BALANCE+50)
...
def test_valid_transaction():
    Transaction.is_valid_transaction(Transaction(Wallet(), 'first_recipient', 50))
